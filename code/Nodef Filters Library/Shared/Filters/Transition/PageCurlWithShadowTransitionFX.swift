...
class PageCurlWithShadowTransitionFX: BaseTransitionFX {
    ...
    let description = "Transitions from one image to another by simulating a curling page, revealing the new image as the page curls. Backside Image is the image that appears on the back of the source image, as the page curls to reveal the target image. Angle is the angle of the curling page. Radius is the radius of the curl. Please refer to the Presets on how to use this filter node. Shadown Amount is the strength of the shadow. Shadown Size is the maximum size in pixels of the shadow. Shadow Extent is the rectagular portion of input image that will cast a shadow."
