...
import RealityKit
import ARKit
class SphereFX: BaseEntityFX {
       
    @Published var timeElapsed:Float = 0.0
    @Published var radius:Float = 0.3
    ...
    let description = "Sphere for Augmented Reality"
    //private var displayEntity: ModelEntity!
    override init()
    {
        let name = "CISphereReality"
        super.init(name)
        desc=description
    }
    ...
    init(photoARView: PhotoARView)
    {
        ...
        let light01Anchor = AnchorEntity(world: [0, 0, 2])
        light01Anchor.addChild(light01)
        photoARView.scene.addAnchor(light01Anchor)
         */
        /*
        let cameraEntity = PerspectiveCamera()
        cameraEntity.camera.fieldOfViewInDegrees = 60
        let cameraAnchor = AnchorEntity(world: [0.0, 0.0, 2.0])
        cameraAnchor.addChild(cameraEntity)
        let cameraTranslation = SIMD3<Float>(5, 5, 0)
         */
    ...
    */
    ...
    func generateObjectWithVideo(_ ciImage: CIImage) {
        ...
        let displayModelEntity = ModelEntity(mesh: MeshResource.generateSphere(radius: radius))
    ...
    func generateObject(_ ciImage: CIImage) {
        ...
        if let cgimg = context.createCGImage(ciImage , from: ciImage.extent) {
            ...
            if let data = processedImage.pngData() {
                let filename = getDocumentsDirectory().appendingPathComponent("sphere"+UUID().uuidString+".png")
                ...
                do {
                    ...
                    //baseMaterial.baseColor = .init(tint: .white.withAlphaComponent(0.999),
                   //                     texture: .init(try! .load(named: "VincentvanGoghSunflowers.jpg")))
                    var material = SimpleMaterial()
                    material.baseColor = MaterialColorParameter.texture(texture)
                    let device = MTLCreateSystemDefaultDevice()!
                    let library = device.makeDefaultLibrary()!
                     let geometryWaterModifier = CustomMaterial.GeometryModifier(named: "geometryWaterModifier2", in: Singleton.getARLibrary())
                     let surfaceWaterShader = CustomMaterial.SurfaceShader(named: "surfaceWaterShader1", in: Singleton.getARLibrary())
