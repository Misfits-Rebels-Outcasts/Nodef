...
@available(iOS 15.0, *)
struct NodeInputDSectionViewX: View {
    ...
    var body: some View {
        ...
        Section(header: Text("Input D"), footer: Text("Select a Step to use as Input D."))
        {
            NavigationLink
            {
                NodeSelectionViewX(nodeType:"inputD",
                ...
                HStack{
                    Text("Step")
                    Spacer()
                    if filterPropertiesViewModel.filterXHolder.filter.inputDAlias == ""{
                        Text("Preceding").foregroundColor(Color.gray)
                    }
                    else{
                        Text("Step "+filterPropertiesViewModel.filterXHolder.filter.inputDAlias).foregroundColor(Color.gray)
                    }
            ...
            HStack{
                ...
                if filterPropertiesViewModel.filterXHolder.filter.inputDAlias == ""{
                    ...
                    Text(filterPropertiesViewModel.filterXHolder.filter.inputDAlias)
                }
            }
        }
    }
