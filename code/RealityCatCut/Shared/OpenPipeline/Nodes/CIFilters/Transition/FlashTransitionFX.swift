...
class FlashTransitionFX: BaseTransitionFX {
    ...
    override func getCIFilter(firstImage: CIImage, secondImage: CIImage) -> CIFilter? {
        ...
        currentCIFilter.setValue(firstImage, forKey: kCIInputTargetImageKey)
        
        let tcenter=CIVector(x:CGFloat(centerX),y:CGFloat(centerY))
        currentCIFilter.setValue(tcenter, forKey: kCIInputCenterKey)
        
        _ = CIVector(cgRect: CGRect(x: CGFloat(x), y: CGFloat(y), width: CGFloat(extentWidth), height: CGFloat(extentHeight)))
