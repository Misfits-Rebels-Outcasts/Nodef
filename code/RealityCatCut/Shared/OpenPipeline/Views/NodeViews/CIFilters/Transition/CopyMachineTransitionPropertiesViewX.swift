...
import SwiftUI
@available(iOS 15.0, *)
struct CopyMachineTransitionPropertiesViewX: View, NodeProperties {

    
    @ObservedObject var fx: CopyMachineTransitionFX = CopyMachineTransitionFX()
    ...
    var body: some View {
        ...
        Section(header: Text("Options")){
            ...
            Slider(value: $fx.opacity, in: 0...3, onEditingChanged: {editing in
                applyFilter(editing)
            })
                .onChange(of: fx.opacity) {  oldValue, newValue in
                    //print("id",fx.id)
                    applyFilter()
                }
        }
