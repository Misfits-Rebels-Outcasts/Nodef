...
class PointillizeFX: FilterX {
       
    @Published var radius:Float = 20.0
    @Published var centerX:Float = 150
    @Published var centerY:Float = 150
    
    let description = "Renders the source image in a pointillistic style. Radius is the radius of the circles in the resulting pattern."
    override init()
    {
        ...
        print("inputRadius")
        if let attribute = CIFilter.pointillize().attributes["inputRadius"] as? [String: AnyObject]
        {
