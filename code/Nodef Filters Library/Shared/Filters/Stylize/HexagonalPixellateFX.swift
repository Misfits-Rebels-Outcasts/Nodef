...
class HexagonalPixellateFX: FilterX {
       
    @Published var scale:Float = 8.0
    @Published var centerX:Float = 150
    @Published var centerY:Float = 150
    
    let description = "Maps an image to colored hexagons whose color is defined by the replaced pixels. Larger Scale values result in larger hexagons."
    ...
    override init()
    {
        ...
        print("inputScale")
        if let attribute = CIFilter.hexagonalPixellate().attributes["inputScale"] as? [String: AnyObject]
        {
