...
class AreaMinimumAlphaFX: FilterX {
    ...
    //let description = "Returns a single-pixel image that contains the color vector with the minimum alpha value for the region of interest."
    let description = "Returns an image that contains the color vector with the minimum alpha value for the region of interest."
    ...
    var areaMinimumAlphaCIFilter: CIFilter?
    //deprecated
    override func getCIFilter(currentImage: CIImage, beginImage: CIImage) -> CIFilter? {
        
        var currentCIFilter: CIFilter
        var areaMinimumAlphaFilter: CIFilter
        
        if ciFilter != nil {
            print("current areaMinimumAlpha")
            currentCIFilter = ciFilter!
            areaMinimumAlphaFilter = areaMinimumAlphaCIFilter!
        } else {
            print("new areaMinimumAlpha")
            areaMinimumAlphaFilter = CIFilter(name: type)!
        ...
        areaMinimumAlphaFilter.setValue(aliasImage, forKey: kCIInputImageKey)
        let extent = CIVector(cgRect: CGRect(x: CGFloat(x), y: CGFloat(y), width: CGFloat(extentWidth), height: CGFloat(extentHeight)))
        areaMinimumAlphaFilter.setValue(extent, forKey: "inputExtent")
        
        var ciColor: CIColor = .black
        if let parentUw = parent{
            var bitmap = [UInt8](repeating: 0, count: 4)
            //ANCHISES
            //parentUw.context!.render(areaMinimumAlphaFilter.outputImage!, toBitmap: &bitmap, rowBytes: 4, bounds: CGRect(x: 0, y: 0, width: 1, height: 1), format: .RGBA8, colorSpace: nil)
            parentUw.getContext().render(areaMinimumAlphaFilter.outputImage!, toBitmap: &bitmap, rowBytes: 4, bounds: CGRect(x: 0, y: 0, width: 1, height: 1), format: .RGBA8, colorSpace: nil)
        ...
        areaMinimumAlphaCIFilter = areaMinimumAlphaFilter
    ...
    override func getCIFilter(_ ciImage: CIImage) -> CIFilter {
        ...
        areaMinimumAlphaFilter.setValue(ciImage, forKey: kCIInputImageKey)
        let extent = CIVector(cgRect: CGRect(x: CGFloat(x), y: CGFloat(y), width: CGFloat(extentWidth), height: CGFloat(extentHeight)))
        areaMinimumAlphaFilter.setValue(extent, forKey: "inputExtent")
