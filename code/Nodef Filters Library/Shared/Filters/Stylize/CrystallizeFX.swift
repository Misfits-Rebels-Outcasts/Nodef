...
class CrystallizeFX: FilterX {
       
    @Published var radius:Float = 20.0
    @Published var centerX:Float = 150
    @Published var centerY:Float = 150
    
    let description = "Creates polygon-shaped color blocks by aggregating source pixel-color values.  The larger the Radius, the larger the resulting crystals."
    ...
    override init()
    {
        ...
        print("inputRadius")
        if let attribute = CIFilter.crystallize().attributes["inputRadius"] as? [String: AnyObject]
        {
